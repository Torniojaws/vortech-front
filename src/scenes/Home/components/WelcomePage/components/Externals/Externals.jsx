...
class Externals extends React.Component {
  render () {
    const externals = [
      ...
      { url: 'https://www.twitter.com/vortechmetal', logo: 'twitter.jpg', name: 'Twitter' },
      { url: 'https://www.youtube.com/vortechband', logo: 'youtube.jpg', name: 'YouTube' }
    ];

    return externals.map((ext) => {
      return (
        <a key={ ext.logo } href={ ext.url }>
          <img className='external' src={ `static/img/external/${ext.logo}` } alt={ ext.name } />
        </a>
      );
    });
  }
